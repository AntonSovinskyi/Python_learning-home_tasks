...
class Counter(threading.Thread):
    ...
    def run(self):
        for each in range(self.rounds):
            self.counter += 1


count = Counter()

inst1 = threading.Thread(target=Counter.run, args=(count, ))
inst2 = threading.Thread(target=Counter.run, args=(count, ))

inst1.start()
inst2.start()

print('Global', counter)
print('Local', count.counter)
